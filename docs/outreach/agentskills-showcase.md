...
The new playground makes four implementation details inspectable:

- **Local parser comparison:** paste or select a SKILL.md and compare the provider's strict and lenient results using its actual parser. Input stays in the page; exported diagnostics omit the raw body. This checks provider parsing, not complete spec conformance or security.
...
This is a discovery/inspection tool, not a new Skills client or a proposal to change the specification. The sample workspace does not scan visitor directories. The optional checker reads only pasted text or an explicitly selected file in the browser; it executes no instructions and hosts no MCP endpoint. The installed tool provides instructions and resource locations; the consuming client controls execution and permissions. Directory definitions do not establish compatibility with every client or skill script.
